...
const Article_Header = (props) => (
  <Card className={`span-12 ${ props.className }`}>
      ...
      By <Link href={{ pathname: '/author', query: { id: article.author._id } }}><a>{article.author.username},</a></Link>  {props.article.author.reporterTitle} | {moment(props.article.updatedAt).format('MMMM D YYYY').toUpperCase()} | {props.article.issue.words}
      </h5>
  <style jsx>{`
